...
class EntityActionMixin:
    """
    Base mixin for entity-related actions.

    Validate/Execute contract (project convention):
    - `validate()` (if implemented by a concrete Action):
        * performs authorization checks (e.g. `check_perm(...)`) and any other checks
          that you may want to skip for alternative auth flows (e.g. anonymous share links).
        * must NOT perform existence checks (use `check_exist()` in execute instead).
    - `execute()`:
        * must ALWAYS call `check_exist()` (directly or indirectly) to ensure referenced
          entity/affix/perm exists before performing DB operations or returning data.

    This convention allows endpoints to choose:
    - standard authenticated flow: init -> validate() -> execute()
    - alternative auth flow (e.g. Share-Token): init (with pre-authorized context) -> execute()
    """
            ...
            for param, value in self.params.items():
                ...
                # Param grander than equal
                    ...
                    if (
                        ...
                        filters[-1] += (
                            f"coalesce((\"{table_name}\".params ->> '{param}')::timestamp >= ({ph})::timestamp, false)"
                        )
                        qp.set_reserved(int(ph.replace('$', '')), value)
                    elif (
                        isinstance(value, int)
