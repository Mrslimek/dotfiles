...
from service54.actions.anonymous_link_actions import AnonymousAffixReadAction, AnonymousAffixListAction
...
class AnonymousAffixList:
    ...
    async def post(
        ...
        # Create anonymous action that bypasses permission checks
        action = AnonymousAffixListAction(data_dict, request, db)
        result = await action.execute()
...
class AnonymousAffixRead:
    ...
    async def post(
        ...
        action = AnonymousAffixReadAction(data.model_dump(), request, db, shared_entity_uuid=entity_uuid)
        result = await action.execute()
