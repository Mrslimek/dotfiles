...
class AnonymousLinkCreate:
    ...
    async def post(
        ...
        """
        partition_uuid = request.headers.get("Partition-Uuid")
        if not partition_uuid:
            raise HTTPException(
                status_code=400,
                detail="Partition-Uuid header is required"
            )
        result = await AnonymousLinkCreateAction(data, request, db, partition_uuid=partition_uuid).execute()
...
class AnonymousAffixList:
    ...
    async def post(
        self,
        request: Request,
        data: AnonymousAffixListValidator,
        share_token: str = Depends(share_token_dependency),
        db: AsyncDatabaseAdapter = Depends(get_db_connection_for_submodule),
    ):
        """
        ...
        # Use entity_uuid from share token if not provided in request
        data_dict = data.model_dump()
        if not data_dict.get("entity_uuid"):
