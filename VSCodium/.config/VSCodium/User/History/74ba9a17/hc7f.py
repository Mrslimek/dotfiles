...
podruchniy_service = PodruchniyService()


@labeler.message(text="<inn>")
async def handle_inn(message: Message, inn: str) -> None:
    if not is_valid_inn_or_ogrn(inn):
        await message.answer(
            "Пожалуйста, введите корректный ИНН (10 или 12 цифр) или ОГРН (13 или 15 цифр)."
        )
        return
    ...
    try:
        result = await podruchniy_service.find_by_id_party(inn)
        response_text = format_dadata_response(result)

    except Exception as e:
        logger.error(f"Error processing identifier {inn}: {e}")
        response_text = "Произошла ошибка при поиске информации. Попробуйте позже."

    await message.answer(response_text)


def format_dadata_response(data: dict) -> str:
...
def is_valid_inn_or_ogrn(value: str) -> bool:
    value = value.strip()
    if not value.isdigit():
        return False
    return len(value) in (10, 12, 13, 15)
