...
async def get_company_handler(message: Message, query: str) -> None:
    try:
        validated_data = CompanyQuery(query=query).model_dump()
    ...
    try:
        response_text = await GetCompanyInfoAction(validated_data.query).execute()

    except Exception as e:
        logger.error(f"Error processing identifier {validated_data.query}: {e}")
