...
async def get_company_handler(message: Message, query: str) -> None:
    ...
    try:
        result = await podruchniy_service.find_by_id_party(validated.query)
...
def validate_ru_inn_or_ogrn(value: str) -> str:
    """
    Валидация ИНН/ОГРН по стандартам РФ:
    - ИНН: Приказ ФНС России № ММВ-7-6/435@
    - ОГРН: Постановление Правительства РФ № 438
    """
    v = value.strip()
